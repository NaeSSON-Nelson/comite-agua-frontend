...
@Component({
  selector: 'app-afiliados-planillas',
  templateUrl: './afiliados-planillas.component.html',
  styles: [
  ]
})
export class AfiliadosPlanillasComponent {
  ...
  ngOnInit(): void {
    ...
    this.pagosService.afiliados.subscribe((res) => {
  ...
  findAll() {
    this.pagosService.findAll(this.dataPaginator).subscribe({
      next: (res) => {
        if (res.OK === false) {
  ...
  dataDetail(id: number) {
    this.router.navigate(['pagos-de-servicio', 'planillas-afiliado'], { queryParams: { id } });
  }
