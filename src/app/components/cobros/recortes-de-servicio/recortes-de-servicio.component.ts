...
@Component({
  selector: 'app-recortes-de-servicio',
  templateUrl: './recortes-de-servicio.component.html',
  styles:[]
})
export class RecortesDeServicioComponent {
  ...
  obtenerLista(){
    ...
    this.recortesForm.reset();
    this.cobrosService.obtenerListaParaRecortes().subscribe(res=>{
      console.log('lista de recortes',res);
      this.loading=false;
      if(res.OK){
        this.data=res.data!;
        this.visibleTable=true;
        if(this.data.length===0){
          this.title='NO HAY AFILIADOS PARA REALIZAR EL CORTE DE SERVICIO'
        }else{
          this.title='';
        }
      }
  ...
  getUbicacion(ubicacion:Ubicacion){
    ...
    return total.concat(ubicacion.barrio ||'')
                .concat(ubicacion.numeroVivienda ||'')
                .concat(`\nMazano: ${ubicacion.manzano}-${ubicacion.numeroManzano}`)
                .concat(`\nN° Lote: ${ubicacion.nroLote}`);
  }

  recortesForm:FormGroup = this.fb.group({
    medidoresCortados:this.fb.array([],[Validators.required]),
  });

  get medidoresArrayForm(){
    return this.recortesForm.get('medidoresCortados') as FormArray;
  }

  addSelect(event:any){
    const asociacion = this.medidoresArrayForm.value.find((med:any)=> med.id === event.data.id);

    if(!asociacion){
      const form = this.fb.group({
        id:[event.data.id,Validators.required],
        tipoCorte:[event.data.tipo,Validators.required]
      })
      this.medidoresArrayForm.push(form);
    }
  }
  dropSelect(event:any){
    const asociacion = this.medidoresArrayForm.value.findIndex((med:any)=> med.id === event.data.id);
  ...
  registrarCortes(){
    console.log('FORM',this.recortesForm);
    console.log('VALUE FORM',this.recortesForm.value);
    if(this.medidoresArrayForm.length===0){
      this.messageService.add({
        severity: 'warn',
        summary: 'ERROR EN VALIDACION',
        detail: `Debe haber seleccionado un corte para registrar`,
        life: 2500,
        icon: 'pi pi-times',
      });
      return;
    }
    if(this.recortesForm.invalid){
      return;
    }
    this.enviarCortesDeServicio(this.recortesForm.value)
  }

  enviarCortesDeServicio(dataForm:any){
   this.confirmationService.confirm({
      message: `¿Está seguro de registrar los cortes de servicio seleccionados?`,
      header: 'Confirmar Registro de deuda',
      icon: 'pi pi-info-circle',
      accept:()=>{
        this.cobrosService.enviarRecortes(dataForm).subscribe(res=>{
          console.log(res);
          if(res.OK){
            this.messageService.add({ severity: 'success', summary:'REGISTRO DE CORTES REALIZADO', detail: res.message });
            this.obtenerLista();
          }
        })
      },
  ...
  columns:ExportColumn[]=[
    ...
    { field: 'motivo', header: 'Motivo del corte', selected: true },
  ...
  exportPDF() {
    ...
    this.data.forEach((perfil,index)=>{
      ...
      perfil.afiliado?.medidorAsociado?.forEach((asc:any,index:number)=>{
        if(index>0){
          ...
          rowAux.push({
            text:asc.tipo==='MULTA'?`${asc.motivo} ${asc.totalPagosRetrasos} meses.`
                                  : asc.tipo==='CIERRE'?asc.motivo
                                  :''
            ,
            alignment:'left'
          })
    ...
    const docDefinition:IPdf.TDocumentDefinitions = {
      ...
      content: [
        { 
          text: 'LISTA DE AFILIADOS PARA CORTE DE SERVICIO DE AGUA',
          style: 'header',
          alignment: 'center',
          margin: [0, 0, 0, 10  ]
        },{
  ...
  pdfMake.createPdf(docDefinition).download(`perfiles-para-corte-servicio-${fechaFormat}.pdf`);
  }
}
