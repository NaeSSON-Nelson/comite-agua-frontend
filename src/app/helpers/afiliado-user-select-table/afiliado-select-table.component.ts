...
@Component({
  selector: 'app-afiliado-user-select-table',
  templateUrl: './afiliado-select-table.component.html',
  styles: [
  ]
})
export class AfiliadoUserSelectTableComponent {
  data: Afiliado[] = [];
  dataSelected!: Afiliado;
  titleTable:string='Tabla Asignacion de cuenta a un afiliado';
  ...
  ngOnInit(): void {
    ...
    this.itemsSub=this.helperService.afiliadosSinUsuario.subscribe((res) => {
  ...
  findAll() {
    this.helperService.findAllAfiliadosSinUsuario(this.dataPaginator).subscribe({
      next: (res) => {
        if (!res) {
