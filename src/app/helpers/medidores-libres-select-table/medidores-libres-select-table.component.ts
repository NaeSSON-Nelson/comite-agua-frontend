...
@Component({
  selector: 'app-medidores-libres-select-table',
  templateUrl: './medidores-libres-select-table.component.html',
  styles: [
  ]
})
export class MedidoresLibresSelectTableComponent {

  data: Medidor[] = [];
  dataSelected!: Medidor;
  titleTable:string='Medidores de agua libres';
  ...
  ngOnInit(): void {
    ...
    this.itemsSub=this.helperService.medidores.subscribe((res) => {
      console.log(res);
  ...
  findAll() {
    this.helperService.findAllMedidoresWithoutAsociacion(this.dataPaginator).subscribe({
      next: (res) => {
        if (!res) {
