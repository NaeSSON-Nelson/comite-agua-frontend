...
@Component({
  selector: 'app-roles-select-table',
  templateUrl: './roles-select-table.component.html',
  styles: [
  ]
})
export class RolesSelectTableComponent {
  data: Role[] = [];
  dataSelected: Role[] = [];
  titleTable:string='Tabla de asignacion de Roles de acceso';
  ...
  ngOnInit(): void {
    ...
    this.helperService.roles.subscribe((res) => {
  ...
  findAll() {
    this.helperService.findAllRoles(this.dataPaginator).subscribe({
      next: (res) => {
        if (!res) {
